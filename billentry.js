...
async function fetchLastBillNumber() {
    try {
        const billsRef = firebase.database().ref('bills');
        const snapshot = await billsRef.orderByKey().limitToLast(1).once('value');
...
async function saveBill() {
    ...
    try {
        ...
        // Save bill under the party
        const partyBillRef = firebase.database().ref(`parties/${partyKey}/bills`).push();
        await partyBillRef.set(billData);
