    ...
    firebase.database().ref('billingOrders').once('value')
        .then(snapshot => {
            if (snapshot.exists()) {
                ...
                // Sort orders by SOB date (newest first)
                orders.sort((a, b) => {
                    const dateA = new Date(a.sobDate || 0);
                    const dateB = new Date(b.sobDate || 0);
                    return dateB - dateA;
                });

                // Display orders
                orders.forEach(order => {
                    const orderElement = createOrderElement(order, order.id);
...
function createOrderElement(order, orderId) {
    ...
    // Format dates (SOB Date and Order Date)
    ...
    const sobDate = formatDate(order.sobDate); // Sent to Billing Date
    const orderDate = formatDate(order.dateTime); // Original Order Date
    
    // Format remarks (if any)
    ...
    orderDiv.innerHTML = `
        <div class="order-header d-flex justify-content-between align-items-center">
            <div>
                ...
                <p>SOB Date: <strong>${sobDate}</strong></p> <!-- Highlight SOB Date -->
            </div>
            <div>
