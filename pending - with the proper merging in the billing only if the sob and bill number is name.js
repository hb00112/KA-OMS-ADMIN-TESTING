...
async function sendToBilling(orderId) {
    try {
        ...
        const sobDate = new Date().toISOString(); // Full ISO string (with time)

        // STEP 1: Extract ONLY the date (YYYY-MM-DD) for comparison
        const sobDateOnly = sobDate.split('T')[0]; // e.g., "2023-07-12"

        // STEP 2: Check for existing billing orders with the same orderNumber
        ...
        // STEP 3: Find an order with the same orderNumber AND same date (ignoring time)
        billingSnapshot.forEach((childSnapshot) => {
            const existingOrder = childSnapshot.val();
            if (existingOrder.sobDate) {
                const existingSobDateOnly = existingOrder.sobDate.split('T')[0]; // Extract date part
                if (existingSobDateOnly === sobDateOnly) { // Compare dates only
        ...
        // STEP 4: Merge or create new billing order
        if (billingOrder) {
            // Merge with existing order (same orderNumber + same date)
            billingOrder = mergeBillingOrders(billingOrder, order, srqValues);
        } else {
            // Create a new billing order (different date or no existing order)
            billingOrder = createBillingOrder(order, srqValues);
            billingOrder.sobDate = sobDate; // Store full ISO string (with time)
            billingOrderKey = billingOrdersRef.push().key;
        }

        // STEP 5: Update pending order and save changes
